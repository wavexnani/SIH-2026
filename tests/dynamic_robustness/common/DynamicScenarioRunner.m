classdef DynamicScenarioRunner < handle
    % DYNAMICSCENARIORUNNER Isolated closed-loop simulation harness for dynamic scenarios
    
    methods (Static)
        function metric = runTrial(scenario_id, trial_id, scenario_builder_fn, cfg_opt)
            ...
            for k = 1:N_steps
                ...
                % Corridor bound extraction at Ego & obstacle horizon
                ...
                % Evaluate scenario obstacle bounds ahead
                ...
                % Event: Vehicle Resumed (Full-Stop vs Dynamic-Yield)
                if metric.corridor_reopened
                    if metric.safe_stop && world.ego.v >= 1.0 && isnan(metric.resume_time_s)
                        metric.vehicle_resumed = true;
                        metric.resume_time_s = t;
                        metric.recovery_mode = 'FULL_STOP';
                        metric.recovery_success = true;
                        metric.full_stop_recovery_latency_s = metric.resume_time_s - metric.corridor_reopen_time_s;
                        metric.recovery_latency_s = metric.full_stop_recovery_latency_s;
                    elseif ~metric.safe_stop && world.ego.v >= 4.0 && isnan(metric.cruising_resumed_time_s)
                        metric.vehicle_resumed = true;
                        metric.cruising_resumed = true;
                        metric.cruising_resumed_time_s = t;
                        if strcmp(metric.recovery_mode, 'NONE')
                            metric.recovery_mode = 'DYNAMIC_YIELD';
                        end
                        metric.recovery_success = true;
                        metric.yield_recovery_latency_s = metric.cruising_resumed_time_s - metric.corridor_reopen_time_s;
                    end
                end
                
                % Separate Footprint Clearance Calculations
                ...
                if metric.vehicle_resumed && c_scen < 0.0
                    metric.post_recovery_collision = true;
                end
            ...
            % Evaluate Overall Trial Success
            if scenario_id == 2 % Scenario 02: Herd Clears Recovery
                % Success requires blockage detection, safe deceleration/stop, zero scenario collision, obstacle clear, and resume
                if metric.blockage_detected && (metric.safe_stop || metric.safe_deceleration) && ...
                   metric.corridor_reopened && metric.vehicle_resumed && ...
                   ~metric.scenario_obstacle_collision && metric.minimum_herd_clearance_m >= 0.0
                    metric.success = true;
                    metric.recovery_success = true;
                else
                    metric.success = false;
                    metric.recovery_success = false;
                end
            else % Scenarios 01, 03, 04, 05, 06
...
% Returns IDs of agents belonging to the scenario obstacle
ids = [];
for i = 1:length(world.agents)
    ...
    if scenario_id == 1 || scenario_id == 2
        ...
        % For scenarios 3-6, all active agents are scenario agents
        ids = [ids, ag.id];
    end
end
if isempty(ids)
