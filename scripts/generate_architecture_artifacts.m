function generate_architecture_artifacts()
    % GENERATE_ARCHITECTURE_ARTIFACTS Generates the authoritative Stateflow SLX & Simulink Dataflow Artifacts
    
    model_name = 'SIH26037_SupervisoryArchitecture';
    
    % 1. Close model if loaded
    if bdIsLoaded(model_name)
        close_system(model_name, 0);
    end
    
    fprintf('Creating authoritative model: %s...\n', model_name);
    ...
    t1 = Stateflow.Transition(chart); t1.Source = s_init; t1.Destination = s_nav; t1.LabelString = '[lead_dx > 0]';
    t2 = Stateflow.Transition(s_nav); t2.Source = s_cruise; t2.Destination = s_follow; t2.LabelString = '[lead_dx <= 40.0 && lead_dx > 28.0]';
    t3 = Stateflow.Transition(s_nav); t3.Source = s_cruise; t3.Destination = s_yield; t3.LabelString = '[min_ttc <= 6.0]';
    t4 = Stateflow.Transition(s_nav); t4.Source = s_follow; t4.Destination = s_overtake; t4.LabelString = '[lead_dx <= 28.0 && W_avail >= 4.10 && min_ttc > 6.0]';
    t5 = Stateflow.Transition(s_nav); t5.Source = s_follow; t5.Destination = s_yield; t5.LabelString = '[min_ttc <= 6.0 || W_avail < 4.10]';
    t6 = Stateflow.Transition(s_nav); t6.Source = s_yield; t6.Destination = s_overtake; t6.LabelString = '[min_ttc > 6.0 && W_avail >= 4.10 && lead_dx <= 28.0]';
    t7 = Stateflow.Transition(s_nav); t7.Source = s_yield; t7.Destination = s_follow; t7.LabelString = '[min_ttc > 6.0 && lead_dx > 28.0]';
    t8 = Stateflow.Transition(s_nav); t8.Source = s_overtake; t8.Destination = s_recover; t8.LabelString = '[lead_dx >= 7.50]';
    t9 = Stateflow.Transition(s_nav); t9.Source = s_overtake; t9.Destination = s_yield; t9.LabelString = '[min_ttc <= 6.0]';
    t10 = Stateflow.Transition(s_nav); t10.Source = s_recover; t10.Destination = s_cruise; t10.LabelString = '[v_ego >= 7.5]';
    
    % High-priority transitions
    t_emerg = Stateflow.Transition(chart); t_emerg.Source = s_nav; t_emerg.Destination = s_emerg; t_emerg.LabelString = '[mpc_status == 0 || min_ttc < 1.0 || clearance <= 0.05]';
    t_stop = Stateflow.Transition(chart); t_stop.Source = s_emerg; t_stop.Destination = s_stop; t_stop.LabelString = '[v_ego < 0.10]';
    t_comp = Stateflow.Transition(chart); t_comp.Source = s_nav; t_comp.Destination = s_complete; t_comp.LabelString = '[x_ego >= 150.0]';
    
    % -------------------------------------------------------------------------
    % 2. Build Surrounding Data-Flow Blocks
    % -------------------------------------------------------------------------
    sub_percep = [model_name '/Perception_Prediction_Risk'];
    add_block('built-in/Subsystem', sub_percep);
    set_param(sub_percep, 'Position', [60, 140, 360, 420]);
    ...
    set_param(sub_planner, 'Position', [900, 140, 1180, 300]);
